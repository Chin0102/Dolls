package cn.chinuy.display.uicomponents.container {
	 ...
	 * @author chin
	 */
	public class BasePanel extends UIComponent implements IContainer, ITransitable {
		
		public static const ContainerResize : String = "ContainerResize";
		...
		public function set clipAndEnableScrolling( value : Boolean ) : void {
			...
			if( changed ) {
				...
				if( clipAndEnableScrolling ) {
					innerContainer.addEventListener( Event.RESIZE, updateMaskSize );
					updateMaskSize();
					innerContainer.mask = thisMask;
					superAddChild( thisMask );
				} else {
					innerContainer.removeEventListener( Event.RESIZE, updateMaskSize );
		...
		protected function updateMaskSize( event : Event = null ) : void {
			thisMask.x = innerContainer.x;
			thisMask.y = innerContainer.y;
			thisMask.graphics.clear();
			thisMask.graphics.beginFill( 0, 0 );
			thisMask.graphics.drawRect( 0, 0, width, height );
			thisMask.graphics.endFill();
			dispatchEvent( new Event( ContainerResize ));
		}
